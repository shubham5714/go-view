      ...
      <n-layout has-sider sider-placement="right">
        <n-layout style="height: 580px; padding-right: 20px">
          <n-tabs v-model:value="editTab" type="card" tab-style="min-width: 100px;">
            <!-- 提示 -->
            <template #suffix>
              <n-text class="tab-tip" type="warning">Tip: {{ EventLifeTip[editTab] }}</n-text>
            </template>
            <n-tab-pane
              ...
              <n-scrollbar trigger="none" style="max-height: 505px">
                <n-collapse class="go-px-3" arrow-placement="right" :default-expanded-names="[1, 2, 3, 4]">
                  <n-collapse-item title="e" :name="1">
                    <n-text depth="3">Arguments received when the lifecycle event is triggered</n-text>
                  </n-collapse-item>
                  ...
                  <n-collapse-item title="components" :name="3">
                    <n-text depth="3"
                      >Map of all component IDs on the screen; use a chart config id to get and control other chart components</n-text
                    >
                    <n-code :code="`{\n  [id]: component\n}`" language="typescript"></n-code>
                  </n-collapse-item>
                  <n-collapse-item title="node_modules" :name="4">
                    <n-text depth="3">Built-in package variables available in the code environment</n-text>
                    <br />
...
const EventLifeName = {
  [EventLife.VNODE_BEFORE_MOUNT]: 'Before render',
  [EventLife.VNODE_MOUNTED]: 'After render'
}

const EventLifeTip = {
  [EventLife.VNODE_BEFORE_MOUNT]: 'Component DOM does not exist yet',
  [EventLife.VNODE_MOUNTED]: 'Component DOM already exists'
}
